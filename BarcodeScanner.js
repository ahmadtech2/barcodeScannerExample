...
export class QRScannerRectView extends Component {
  ...
  static defaultProps = {
    ...
    hintText: 'Please scan your barcode in the scanning area',
  };
...
export default class QRScannerView extends Component {
  ...
  constructor(props) {
    super(props);
    // 避免频繁触发扫描回调
    this.onScanResult = throttle(this.onScanResult, this.props.scanInterval, {
      maxWait: 0,
      trailing: false,
    });
    this.state = {
    ...
    // this.FromPage = props.route.params.FromPage;
    // this.FromInput = props.route.params.FromInput;
  }
  ...
  onScanResult = e => {
    ...
    console.warn('onScanResult', e.data)
    // let FromPage = this.FromPage;
    // let FromInput = this.FromInput;
    // if (FromPage == 'Dashboard') {
    //   this.props.navigation.dispatch(
    //     StackActions.replace('ScannedScreen', {
    //       input: e.data,
    //     }),
    //   );
    // } else {
    //   this.props.route.params.returnBarcode(FromInput, e.data);
  ...
  render() {
    return (
      <RNCamera
        ...
        captureAudio={false}
        onBarCodeRead={e => this.onScanResult(e)}
        type={
          this.state.userFront
        ...
        {/*绘制顶部标题栏组件*/}
        {/* {renderHeaderView && ( */}
          <View style={[styles.topContainer]}>{this.renderHeaderView()}</View>
         {/* )} */}

        {/*绘制底部操作栏*/}
        {/* {renderFooterView && ( */}
          <View style={[styles.bottomContainer]}>{this.renderFooterView()}</View>
        {/* )} */}
      </RNCamera>
    );
  }
